...
import de.incunabulum.owl4java.jmodel.JClassRestriction;
...
import de.incunabulum.owl4java.jmodel.utils.DebugUtils;
import de.incunabulum.owl4java.jmodel.utils.NamingUtils;
import de.incunabulum.owl4java.utils.NamespaceUtils;

public class Owl2JModelReader {

	// XXX: equivalent classes, properties ignored

	private static Log log = LogFactory.getLog(Owl2JModelReader.class);
	...
	protected void createJRestriction(Restriction res, OntClass cls,
			OntProperty prop) {
		JClassRestriction jRestriction = new JClassRestriction(jmodel
				.getJClass(cls.getURI()), jmodel.getJProperty(prop.getURI()));
		...
		if (res.isHasValueRestriction()) {
			jmodel.ontResourceErrors.add(res);
	...
	protected void createProperty(OntProperty ontProperty) {
		...
		while (rIt.hasNext()) {
			...
			if (jmodel.uri2class.containsKey(range.getURI())) {
				...
				prop.addObjectRange(jmodel.getJClass(range.getURI()));
				...
				prop.addDatatypeRange(range.getURI());
	...
	protected void abortIfIdenticalWithBaseThing(OntClass ontClass) {
		...
		String baseThingUri = jmodel.getNamespace("") + JModel.THINGNAME;
	...
	protected void createJClassish(OntClass ontClass) {
		log.info(DebugUtils.logClass(ontClass));
	...
	public JModel generateJModel(OntModel model) {
		...
		// find all relevant classes and populate the JModel with the
		...
		// Notes: Use assert(),
		// Notes: xsd to java -> custom mapper?
	...
	protected void handlePropertyRanges() {
		...
		while (propertyIt.hasNext()) {
			...
			if (prop.getObjectRange().size() < 2)
				continue;

			log.info(DebugUtils.logProperty(prop)
					+ ": Found multiple range. Replacing with UnionClass ");

			// Create a new anonymous class as the union of the ranges
			anonClassCounter++;
			String anonClassName = anonClassBase + anonClassCounter;
			...
			if (!jmodel.hasJClass(anonClassUri))
				jmodel.createJClass(anonClassName, anonClassUri,
						basePackage);
			JClass newCls = jmodel.getJClass(anonClassUri);


			// register range as super classes in class
			for (JClass superCls : prop.getObjectRange()) {
				superCls.addSubClass(newCls);
				log.debug(NamingUtils.toLogName(newCls)
						+ ": Registering super class "
						+ superCls.getJavaPackageName() + "."
						+ superCls.getName());
			}

			// reset range for property to new union class
			prop.getObjectRange().clear();
			prop.addObjectRange(newCls);
	...
	protected void handleAnonymousClasses() {
		...
		for (int i = 0; i < list.size(); i++) {
			...
			if (ontClass.isComplementClass()) {
				jmodel.ontResourceErrors.add(ontClass);
			...
			if (ontClass.isIntersectionClass()) {
				jmodel.ontResourceErrors.add(ontClass);
				log
						.warn("Found non restriction anonymous class: Subclasses rergistered "
								+ "as sub classes of OwlThing");
				JClass thingCls = jmodel
						.getJClass(jmodel.baseThingUri);
	...
	protected void createBaseClassishObjects() {
		// create our base object "Thing" and add as class
		String thingUri;
		String thingName = JModel.THINGNAME;
	...
	protected void handleRestrictions() {
		...
		while (it.hasNext()) {
			...
			if (cls.isRestriction()) {
				log.info(DebugUtils.logRestriction(cls.asRestriction()));
